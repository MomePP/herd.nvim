...
describe('herd init', function()
  ...
  it('toggle arms the exit watcher in native mode', function()
    ...
    local watched, watched_origin
    Watch.start = function(a, origin) watched, watched_origin = a, origin end
    ...
    -- the watcher owns the return trip now, so it gets the editor tab
    assert.are.equal('w6:t1', watched_origin)
  end)
